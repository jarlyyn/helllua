...
do_check=function(checkcallback)
	...
	elseif itemsnum("Gold")<(tonumber(GetVariable("goldmin"))) then
		busytest(c_qugold)
	elseif GetVariable("pfm")=="shot" and itemsnum("ÀÇÑÀ¼ı")<10 then
		item["go"]("ÀÇÑÀ¼ı",30,checkcallback,checkcallback)
	elseif GetVariable("pfm")=="shot" and itemsnum("µã½ğÅÌÁú¹­")==0 and itemsnum("³¤¹­")==0 and itemsnum("¶Ì¹­")==0 then
...
checknuqi=function(nuqiok,nuqifial)
	if ((me.score.xingge=="ĞÄºİÊÖÀ±")or(me.score.xingge=="¹âÃ÷ÀÚÂä"))and(tonumber(GetVariable("neilimin"))>1000)and((me.hp.nuqi~="Å­»ğÖĞÉÕ")and(me.hp.nuqi~="Êú·¢³å¹Ú")) then
		run("baofa")
		busytest(nuqiok)
		return true
	end
